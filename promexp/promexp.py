...
class PrometheusExporter:
    ...
    def set_all(self, name_vals: dict[str, float], labels: dict[str, str]) -> None:
        """Convenience function to set many metrics at once with the same
        labels"""

        for name, value in name_vals.items():
            self.set(name, labels, value)

    def clear(self, metric_name: str | None = None) -> None:
        """Remove all metric instances"""

        with self._lock:
            if metric_name:
                self._prom[metric_name].clear()
            else:
